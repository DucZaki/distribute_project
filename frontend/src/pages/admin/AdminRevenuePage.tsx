...
export function AdminRevenuePage() {
  ...
  const [monthly, setMonthly] = useState({
    labels: [] as string[],
    data: [] as number[],
    rows: [] as Array<{ period: number; amount: number }>,
  });
  ...
  useEffect(() => {
    if (tab === "month")
      ensureChart("month", monthChartRef.current, monthly.labels, monthly.data);
    if (tab === "week")
      ensureChart("week", weekChartRef.current, weekly.labels, weekly.data);
    if (tab === "year")
      ensureChart("year", yearChartRef.current, yearly.labels, yearly.data);
  }, [tab, year, monthly, weekly, yearly]);

  const activeRows =
    tab === "month" ? monthly.rows : tab === "week" ? weekly.rows : yearly.rows;
  const periodLabel =
    tab === "month" ? "Tháng" : tab === "week" ? "Tuần" : "Năm";
              ...
              <tbody>
                {transactions.map((row) => (
                  <tr key={row.bookingId}>
                    <td className="px-4 py-3 text-muted fw-bold">
                      #{row.bookingId}
                    </td>
                    <td className="py-3">
                      {row.userId ? (
