...
export type RevenueBreakdown = {
  ...
  monthlyData?: number[]
  monthlyRows?: Array<{ period: number; amount: number }>
  weeklyLabels?: string[]
  weeklyData?: number[]
  weeklyRows?: Array<{ period: number; amount: number }>
  yearlyLabels?: string[]
  yearlyData?: number[]
  yearlyRows?: Array<{ period: number; amount: number }>
  labels?: string[]
  data?: number[]
}

export function getMonthlyRevenue(year: number) {
  return apiFetch<RevenueBreakdown>(`/admin/dashboard/revenue/monthly?year=${year}`, {}, true)
}

export function getRevenueDetail(year: number) {
  return apiFetch<RevenueBreakdown>(`/admin/dashboard/revenue/detail?year=${year}`, {}, true)
}
